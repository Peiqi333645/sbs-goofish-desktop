...
def test_generic_category_query_keeps_ranked_marketplace_results():
    assert title_matches_search_keyword("相机", "索尼 A7M4 全画幅微单")
    assert title_matches_search_keyword("相机", "佳能 R6 二代 单机身")
...
def test_filter_only_enforces_distinctive_model_tokens():
    items = [
        {"商品标题": "康泰时 G1 绿标"},
        {"商品标题": "康泰时 G2 银色"},
    ]
    assert filter_search_items("康泰时 G1", items) == [items[0]]
    assert filter_search_items("相机", items) == items
