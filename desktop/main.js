...
function findFreePort() {
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      const selectedPort = typeof address === "object" ? address.port : 8765;
...
function waitForBackend(timeoutMs = 60000) {
  ...
  return new Promise((resolve, reject) => {
    const retry = () => {
      if (Date.now() - startedAt >= timeoutMs) reject(new Error("后台服务启动超时"));
...
function startBackend() {
  ...
  if (!fs.existsSync(executable)) throw new Error(`找不到后台程序：${executable}`);
...
async function createWindow() {
  ...
  await mainWindow.loadURL(`http://127.0.0.1:${port}/?desktopVersion=0.1.1`);
  mainWindow.show();
}
